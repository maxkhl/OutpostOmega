...
namespace OutpostOmega.Server.Network
{
    ...
    public class Client
    {
        ...
        public Client(string Username, Host Host, NetConnection Connection)
        {
            ...
            {
                if (typeof(Mind).IsAssignableFrom(gameObject.GetType()) && ((Mind)gameObject).Username == Username)
                {
                    this.Mind = ((Mind)gameObject);
            ...
            if (this.Mind == null)
            {
                ...
                this.Mind = new Mind(Host.networkHandler.World);
                ...
                case (byte)Command.Data:
                    switch(subType)
                    {
                        ...
                        case (byte)SecondCommand.InputMouseDelta:
                            ...
                            var newMS = new Game.Tools.MouseState();
                            newMS.ElapsedTime = Clock - im.ReadDouble();
                            newMS.LeftKey = im.ReadBoolean();
                            newMS.MiddleKey = im.ReadBoolean();
                            newMS.RightKey = im.ReadBoolean();
                            //newMS.X = im.ReadInt32();
                            //if(newMS.X > 0)
                            //{ }
                            //newMS.Y = im.ReadInt32();
                            this.Mind.SimulatedMouseInput = newMS;

                            break;
                        case (byte)SecondCommand.Position:                            
                            this.Mind.Mob.SetPosition(
                                new Jitter.LinearMath.JVector(
                                im.ReadFloat(),im.ReadFloat(),im.ReadFloat()));
                            break;
                        case (byte)SecondCommand.Orientation:
                            this.Mind.Mob.Orientation =
