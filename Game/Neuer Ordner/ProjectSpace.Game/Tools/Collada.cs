...
namespace OutpostOmega.Game.Tools
{
    static class Collada
    {
        ...
        {
            public JVector Position, Normal;
            public JVector2 TexCoord;
            public Color Color;
            //public Vector3 Tangent, BiTangent;
            public byte Texture;

            public static readonly int Stride = Marshal.SizeOf(default(Vertex));
            public static readonly int ColorSize = Marshal.SizeOf(default(Color));
        }
        ...
        public static List<Mesh> ReadModel(string File)
        {
            ...
            foreach (Grendgine_Collada_Geometry geometry in Model.Library_Geometries.Geometry)
            {
                ...
                // Build VertexArray
                #region VERTARRAY
                modelMesh.Vertices = new Vertex[VertexPositions.Count / 3];
                for (int i = 0; i < VertexPositions.Count; i += 3)
                {
                    var newVert = new Vertex()
                    {
                        Position = new JVector(vertArray[i], vertArray[i + 1], vertArray[i + 2]),
        ...
        public static Jitter.Collision.Shapes.TriangleMeshShape ShapeFromMesh(string Model, string Mesh)
        {
            var meshs = ReadModel(Model);
            Jitter.Collision.Octree octree = null;
            foreach(var mesh in meshs)
                if(mesh.Name == Mesh)
                {
                    ...
                    List<Jitter.Collision.TriangleVertexIndices> Triangles = new List<Jitter.Collision.TriangleVertexIndices>();
                    var indices = mesh.Indices[Grendgine_Collada_Input_Semantic.VERTEX];
                    for(int i = 0; i < indices.Length; i+=3)                        
                        Triangles.Add(new Jitter.Collision.TriangleVertexIndices() { 
                            I0 = (int)indices[i],
                            I1 = (int)indices[i+1],
                            I2 = (int)indices[i+2]});


                    octree = new Jitter.Collision.Octree(Positions, Triangles);
                }

            if (octree != null)
                return new Jitter.Collision.Shapes.TriangleMeshShape(octree);
            else
                return null;
        }
    }
