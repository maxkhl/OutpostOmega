...
namespace OutpostOmega.Game.Tools
{
    static public class Easing
    {
        ...
        /// <summary>
        /// Eases the given value by using the specified function.
        /// </summary>
        ...
        public static float Ease(EaseFunction function, float t, float b, float c, float d)
        {
            MethodInfo method = typeof(Easing).GetMethod(function.ToString(), BindingFlags.Public | BindingFlags.Static);
            
            if (method != null)
                return b + (float)(double)method.Invoke(null, new object[4] { (double)t, (double)0, (double)c - (double)b, (double)d });
            else
                return (float)Linear(t, b, c, d);
